...
# ==========================================
# ⚙️ CONFIGURACIÓN (ADAPTADA A TU NUEVO TRAINING)
# ==========================================
SR = 16000              
HOP_LENGTH = 512        
MIN_NOTE = 'A0'
...
# --- CAMBIO DETECTADO: 4 ARMONICOS DE ENTRADA ---
HARMONICS = [0.5, 1, 2, 3] 

# Umbrales de Inferencia (Ajustables)
THRESHOLD_ONSET = 0.35  
THRESHOLD_FRAME = 0.6   # Puedes probar a subir a 0.6 si notas suciedad
THRESHOLD_OFFSET = 0.4  

# ==========================================
# 1. ARQUITECTURA DEL MODELO (COINCIDE CON TU TRAINING)
# ==========================================
...
class HDConv(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # Según tu training pegado, usas los armónicos estándar aquí
        harmonics = [1, 2, 3, 4] 
...
class AcousticModel(nn.Module):
    def __init__(self, in_channels, base_channels):
        super().__init__()
        # Se adaptará automáticamente a in_channels=4
...
class HPPNet(nn.Module):
    # CAMBIO: Default in_channels=4 para coincidir con tu nuevo HCQT
...
# ==========================================
# 2. PREPROCESAMIENTO HCQT (4 CANALES)
# ==========================================
def compute_input_tensor(audio_path):
    print(f"🔄 Cargando audio y calculando HCQT (4 Canales): {audio_path} ...")
    ...
    # Bucle actualizado para HARMONICS = [0.5, 1, 2, 3]
    for h in HARMONICS:
        current_fmin = base_fmin_hz * h
        
        # Lógica de seguridad idéntica a tu script de Preprocesamiento
    ...
    # Tensor para inferencia: (Batch=1, Channels=4, Freq=88, Time)
    tensor = torch.tensor(hcqt_np).permute(2, 1, 0).unsqueeze(0).float()
    
    return tensor

# ==========================================
# 3. MATRIX TO MIDI (PEAK PICKING OPTIMIZADO)
# ==========================================
...
# ==========================================
def main():
    print("\n🎹 --- INFERENCIA HPPNET 4-CANALES (UPDATED) --- 🎹")
    
    output_dir = Path("midi_output_4h")
    ...
    print(f"\n🔧 Cargando modelo (In=4 Ch) en {DEVICE}...")
    
    # INICIALIZACIÓN CON 4 CANALES
    ...
    try:
        ...
        print(f"❌ Error cargando pesos (verificar si el modelo fue entrenado con 4 canales): {e}")
        return
