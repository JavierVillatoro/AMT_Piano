...
PATIENCE_LR = 4           # Subimos un poco la paciencia para dar tiempo a InstanceNorm
FACTOR_LR = 0.5           

# Umbrales (Ajustados para esta arquitectura)
THRESHOLD_ONSET = 0.4     # Un poco más bajo para capturar más notas
THRESHOLD_FRAME = 0.5
THRESHOLD_OFFSET = 0.4    # Subimos un poco respecto a 0.3 para mejorar precisión
...
# ==========================================
# 2. ARQUITECTURA MEJORADA (InstanceNorm)
# ==========================================
...
class ConvBlock(nn.Module):
    def __init__(self, in_c, out_c, kernel_size=(3,3), padding=(1,1)):
        ...
        self.net = nn.Sequential(
            ...
            # CAMBIO CRÍTICO: InstanceNorm2d con affine=True
            # Esto estabiliza el entrenamiento con Batch Size pequeño (4)
...
class AcousticModel(nn.Module):
    def __init__(self, in_channels, base_channels):
        ...
        # CAMBIO CRÍTICO: InstanceNorm aquí también
...
class HPPNet(nn.Module):
    def __init__(self, in_channels=3, base_channels=24, lstm_hidden=48):   #ANTES 32
        super().__init__()
...
def plot_training_history(csv_path="training_log_2.csv"):
    if not os.path.exists(csv_path): return
    df = pd.read_csv(csv_path)
    
    sns.set_theme(style="whitegrid", context="paper")
    fig, axes = plt.subplots(2, 2, figsize=(16, 10))
    fig.suptitle('HPPNet (InstanceNorm) Metrics', fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig("training_results_optimized.png", dpi=200)
...
if __name__ == "__main__":
    print(f"\n🚀 HPPNET-SP FINAL OPTIMIZED Training ({DEVICE})")
    ...
    model = HPPNet(in_channels=3, lstm_hidden=48).to(DEVICE)   #ANTES 32
    optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE)
    scheduler = ReduceLROnPlateau(optimizer, mode='max', factor=FACTOR_LR, patience=PATIENCE_LR)
    scaler = torch.amp.GradScaler('cuda') 
    
    # --- CAMBIO ESTRATÉGICO DE LOSS ---
    # 1. Onset: Seguimos con Focal Loss (funciona genial para eventos puntuales)
    crit_onset = FocalLoss(alpha=0.75, gamma=2.0).to(DEVICE)
    
    # 2. Frame: Volvemos a BCE con peso alto.
    # Esto recupera la "solidez" de las notas del modelo antiguo.
    # Evita que la nota se rompa (jitter) en medio.
    crit_frame = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([10.0]).to(DEVICE))
    
    # 3. Offset: Volvemos a BCE con peso moderado.
    # Focal Loss era demasiado agresiva y creaba falsos positivos.
    # BCE es más "suave" y subirá el Recall del Offset.
    ...
    try:
        for epoch in range(FINAL_EPOCHS):
            ...
            with tqdm(train_loader, desc=f"Ep {epoch+1}/{FINAL_EPOCHS}", leave=False) as bar:
                for batch in bar:
                    ...
                    with torch.amp.autocast('cuda'):
                        ...
                        # Pesos ajustados para equilibrio
            ...
            print("-" * 80)
            print(f"🏁 Epoch {epoch+1} Results (InstanceNorm + MixedLoss):")
        ...
        log_file.close()
        plot_training_history("training_log_2.csv")