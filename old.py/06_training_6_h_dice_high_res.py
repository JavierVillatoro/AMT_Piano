...
# Umbrales
THRESHOLD_ONSET = 0.3    # 0.3 ????
THRESHOLD_FRAME = 0.25     # 0.3 ????
THRESHOLD_OFFSET = 0.35    # 0.35 para ser mas sensibles al final de la nota 
...
class PianoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            return {"hcqt": torch.zeros(4, 88, SEGMENT_FRAMES), "onset": z, "frame": z, "offset": z, "velocity": z}
...
class HDConv(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        for h in harmonics:
            if h == 1:
                d = 0    #ANTES 1 , HABIA INCONSISTENCIA 
            else:
            ...
            self.convs.append(nn.Conv2d(
                ...
                padding=(d, 1), 
                dilation=(d, 1),
                bias=False #AÑADO ESTO para problema ruido 
            ))
...
# ==========================================
def refine_time(output_curve, center_frame):
    """
    Implementa Eq (10) y (11) del paper para High-Resolution.
    Calcula el desplazamiento exacto (shift) desde el center_frame.
    """
    # Protecciones de límites
    if center_frame <= 0 or center_frame >= len(output_curve) - 1:
        return center_frame
    
    # A (izq), B (centro/pico), C (der)
    y_a = output_curve[center_frame - 1]
    y_b = output_curve[center_frame]
    y_c = output_curve[center_frame + 1]

    # Evitar división por cero si el modelo es muy seguro (y_b muy alto, y_a/y_c muy bajos)
    if (y_b - y_a) < 1e-6 or (y_b - y_c) < 1e-6:
        return center_frame

    # Lógica del paper (Sección III-D, Proof Fig 4)
    # x_b - x_a siempre es 1 (un frame de distancia)
    
    if y_c > y_a:
        # El pico real está hacia la derecha (hacia C)
        # Eq (10): BH = ((x_b - x_a) / 2) * ((y_c - y_a) / (y_b - y_a))
        shift = 0.5 * ((y_c - y_a) / (y_b - y_a))
    else:
        # El pico real está hacia la izquierda (hacia A)
        # Eq (11): BH = ((x_c - x_b) / 2) * ((y_a - y_c) / (y_b - y_c))
        # Nota: el paper usa distancias positivas, aquí queremos el signo negativo para ir atrás
        shift = -0.5 * ((y_a - y_c) / (y_b - y_c))
        
    # Clamp del shift para que no salte más de medio frame (por seguridad)
    shift = np.clip(shift, -0.5, 0.5)
    
    return center_frame + shift
def tensor_to_notes(onset_pred, frame_pred, offset_pred, velocity_pred, t_onset, t_frame, t_offset):
    notes = []
    for pitch in range(88):
        # 1. Peak Picking básico para encontrar candidatos
        peaks, _ = find_peaks(onset_pred[:, pitch], height=t_onset, distance=2)
        
        for onset_idx in peaks:
            # --- NUEVO: REFINAMIENTO DE ONSET ---
            # Usamos la curva de regresión para calcular el tiempo exacto
            precise_onset_frame = refine_time(onset_pred[:, pitch], onset_idx)
            onset_time = precise_onset_frame * HOP_LENGTH / SR
            # ------------------------------------

            # Validación de sustain (igual que antes)
            check_frame = min(onset_idx + 1, frame_pred.shape[0] - 1)
            if frame_pred[check_frame, pitch] < t_frame:
                continue

            # Búsqueda del Offset
            end_idx = onset_idx + 1
            offset_candidate_idx = -1
            
            while end_idx < frame_pred.shape[0]:
                # Si encontramos un pico de offset fuerte
                if offset_pred[end_idx, pitch] > t_offset:
                    # Verificamos si es pico local
                    if (end_idx > 0 and end_idx < len(offset_pred)-1 and 
                        offset_pred[end_idx, pitch] > offset_pred[end_idx-1, pitch] and
                        offset_pred[end_idx, pitch] > offset_pred[end_idx+1, pitch]):
                        offset_candidate_idx = end_idx
                        break # Encontramos el offset explícito
                
                # Si el frame cae por debajo del umbral, se acabó la nota
                if frame_pred[end_idx, pitch] < t_frame:
                    offset_candidate_idx = end_idx
                    break
                
                end_idx += 1
            
            # Si no encontramos final, cortamos al final del clip
            if offset_candidate_idx == -1:
                offset_candidate_idx = min(end_idx, frame_pred.shape[0]-1)

            # --- NUEVO: REFINAMIENTO DE OFFSET ---
            # Si el final fue dictado por un OFFSET REGRESSION (no por caída de frame)
            if offset_pred[offset_candidate_idx, pitch] > t_offset:
                precise_offset_frame = refine_time(offset_pred[:, pitch], offset_candidate_idx)
            else:
                # Si fue por caída de frame, no podemos refinar con la fórmula triangular
                precise_offset_frame = offset_candidate_idx
                
            offset_time = precise_offset_frame * HOP_LENGTH / SR
            # -------------------------------------

            if offset_time - onset_time > 0.05: # Filtro de notas muy cortas (50ms)
                vel = 0
                if velocity_pred is not None:
                    # Velocity se toma del pico del onset
                    vel = velocity_pred[onset_idx, pitch]
...
if __name__ == "__main__":
    ...
    crit_onset = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([15.0]).to(DEVICE)) #NUEVO PARA HIGH_RES
    crit_frame = ComboLoss(bce_weight=4.0).to(DEVICE)
    crit_offset = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([15.0]).to(DEVICE))
            ...
            with tqdm(train_loader, desc=f"Ep {epoch+1}/{FINAL_EPOCHS}", leave=False) as bar:
                for batch in bar:
                    ...
                    with torch.amp.autocast('cuda'):
                        ...
                        loss = (2*l_on) + l_fr + l_off + (0.5*l_vel) 
            ...
            if should_validate:
                ...
                with torch.no_grad():
                    for batch in val_loader:
                        ...
                        v_loss += ((2.0 * l_on) + l_fr + l_off + (0.5 * l_vel)).item()
