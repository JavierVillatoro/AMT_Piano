...
DATA_PATH = Path("/kaggle/input/datasets/javiervillatoro/maestro-cqt-pedal-20/processed_data_cqt_pedal")
CHECKPOINT_PATH = Path("/kaggle/working/best_hppnet_phase3_pedal.pth") # Volver a dejarlo como antes
...
FINAL_EPOCHS = 50         
LEARNING_RATE = 0.0003    # Antes 0.0006, añadido por pedal    
...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        # 🔹 FORZAR FP32 PARA EVITAR DESBORDAMIENTO
        inputs = inputs.float()
...
class DiceLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        # 🔹 FORZAR FP32 ANTES DE SUMAR 700,000 PÍXELES
        ...
        for epoch in range(FINAL_EPOCHS):
            ...
            with tqdm(train_loader, desc=f"Ep {epoch+1}/{FINAL_EPOCHS}", leave=False) as bar:
                for batch in bar:
                    ...
                    with torch.amp.autocast('cuda'):
                        ...
                        l_vel = (crit_vel(torch.sigmoid(p_vel).float(), targets['velocity'].float()) * mask.float()).sum() / (mask.float().sum() + 1e-4)
            ...
            if should_validate:
                ...
                if frame_f1 > best_frame_f1:
                    ...
                    torch.save(state_dict, "best_hppnet_phase3_pedal_2.pth")
