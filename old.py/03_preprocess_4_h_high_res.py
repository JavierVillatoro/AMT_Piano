...
HARMONICS = [0.5, 1, 2, 3]   #CAMBIO A 4 HARMONICOS 
...
def compute_labels(midi_path, num_frames):
    ...
    for note in pm.instruments[0].notes:
        ...
        #end_frame_float = note.end / time_per_frame + 1.0   ANTES
...
# Modifica la definición para aceptar 'offsets'
def save_verification_plot(audio, hcqt, onsets, offsets, frames, velocities, file_id, save_path):
    ...
    if duration_sec > zoom_sec:
        ...
        offsets_cut = offsets[:max_frame] # <--- Recortamos offsets
        ...
        frames_cut = frames
        onsets_cut = onsets
        offsets_cut = offsets # <--- Asignamos offsets
        vel_cut = velocities
    ...
    # CAMBIO: Aumentamos a 6 subplots y aumentamos un poco el tamaño vertical
    ...
    ax[4].imshow(offsets_cut.T, aspect='auto', origin='lower', extent=extent_cut, cmap='Blues') # Uso 'Blues' para diferenciar
    # ---------------------------
    
    # Movemos Velocity al índice 5
...
def procesar_dataset():
    ...
    output_base = Path("processed_data_HPPNET_4_h_high_res") 
    ...
    print("\n🏆 ¡Proceso finalizado!")
    print("⚠️  IMPORTANTE: Ahora tu modelo debe tener in_channels=4.")
